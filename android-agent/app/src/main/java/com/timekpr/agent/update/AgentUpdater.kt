...
class AgentUpdater(private val context: Context) {
    ...
    fun performUpdate(request: AgentUpdateRequest): UpdateResult {
        ...
        val apkFile = File(updateDir, "timekpr-android-agent-$safeTag.apk")
    ...
    private fun installApk(apkFile: File) {
        ...
        try {
            apkFile.inputStream().use { input ->
                session.openWrite("timekpr-update", 0, apkFile.length()).use { output ->
                    input.copyTo(output)
    ...
    companion object {
        ...
        private const val UPDATE_DIR = "timekpr-update"
    }
}

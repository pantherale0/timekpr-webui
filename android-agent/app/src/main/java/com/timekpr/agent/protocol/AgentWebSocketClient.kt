...
class AgentWebSocketClient(
    ...
    suspend fun runSession(config: AgentConfig, mode: SessionMode): SessionResult {
        ...
        if (!fcmToken.isNullOrBlank()) {
            TimeKprApplication.from(context).configStore.saveFcmToken(fcmToken)
        }
            ...
            "challenge" -> {
                ...
                webSocket.send(
                    AgentMessages.register(
                        config.systemId,
                        hmacSha256(agentToken, challenge + config.systemId),
                    ),
                )
            }
            "auth_result" -> {
    ...
    private fun sendPolicySyncCheck(webSocket: WebSocket) {
        val revisions = TimeKprApplication.from(context).domainPolicyStore.sourceRevisions

...
import com.guardian.agent.discovery.PackageChangeMonitor
...
class UsageMonitorService : Service() {
    ...
    private suspend fun monitorLoop() {
        ...
        while (scope.isActive) {
            ...
            if (callingUserId != 0) {
                ...
                val timeLimitsFile = java.io.File(dataDir, "shared_prefs/guardian_time_limits.xml")
                val domainPolicyFile = java.io.File(dataDir, "shared_prefs/guardian_domain_policy.xml")
                val appPoliciesFile = java.io.File(dataDir, "shared_prefs/guardian_app_policies.xml")
                val deviceRestrictionsFile = java.io.File(dataDir, "shared_prefs/guardian_device_restrictions.xml")
                ...
                if (lastTimeLimitsModified == 0L) {
                    ...
                    val app = GuardianApplication.from(applicationContext)
    ...
    private fun enforceManagedCapabilities(userContext: Context) {
        ...
        if (SecondaryUserProvisioner.isManagedSecondaryUser(userContext)) {
            GuardianApplication.from(userContext).domainPolicyStore.restore()
        }

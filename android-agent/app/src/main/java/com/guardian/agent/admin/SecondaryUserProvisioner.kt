...
import com.guardian.agent.monitor.UsageAlertForwarder
import com.guardian.agent.monitor.UsageAlertRelayActivity
...
object SecondaryUserProvisioner {
    ...
    const val AFFILIATION_ID = "com.guardian.agent.affiliated"
    ...
    fun setupProvisionedUser(primaryContext: Context, userId: Int) {
        if (userId == 0) return
        val configStore = GuardianApplication.from(primaryContext).configStore

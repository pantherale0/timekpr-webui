...
object CrossUserStoreSync {
    ...
    private val REPLICATED_PREFS = listOf(
        "guardian_agent_config",
        "guardian_app_policies",
        "guardian_time_limits",
        "guardian_domain_policy",
        "guardian_device_restrictions",
        "guardian_enforcement",
        "guardian_profile_provisioning",
    )
    ...
    private fun copySharedPreferencesCrossUser(from: Context, to: Context, name: String) {
        to.getSharedPreferences("_guardian_sync_bootstrap", Context.MODE_PRIVATE)
        val source = from.getSharedPreferences(name, Context.MODE_PRIVATE)
        if (source.all.isEmpty()) return
